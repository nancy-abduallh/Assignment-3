        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch + 1, max_epochs + 1):
                    ...
                    # --- MODIFIED: Save checkpoint every epoch (Instruction 9) ---
                    if self._checkpoint_dir:
                        if self.settings.local_rank in [-1, 0]:
                            self.save_checkpoint()
                    # The original condition: if epoch > (max_epochs - 10) or save_every_epoch or epoch % 10 == 0:
                    # is replaced by unconditional saving for every epoch.
                    # --- END MODIFIED ---
